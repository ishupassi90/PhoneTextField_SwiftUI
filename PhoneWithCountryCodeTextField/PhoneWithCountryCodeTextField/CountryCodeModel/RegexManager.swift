...
final class RegexManager {
    ...
    func regexMatches(_ pattern: String, string: String) throws -> [NSTextCheckingResult] {
        do {
            ...
            let matches = currentPattern.matches(in: internalString, range: .init(location: 0, length: internalString.count))
            return matches
        } catch {
            throw PhoneNumberError.generalError
        }
    ...
    func replaceStringByRegex(_ pattern: String, string: String, template: String = "") -> String {
        do {
            ...
            if matches.count == 1 {
                let range = regex.rangeOfFirstMatch(in: string, range: .init(location: 0, length: string.count))
                if range != nil {
                ...
                replacementResult = regex.stringByReplacingMatches(in: string, range: .init(location: 0, length: string.count), withTemplate: template)
            }
            return replacementResult
        } catch {
            return string
