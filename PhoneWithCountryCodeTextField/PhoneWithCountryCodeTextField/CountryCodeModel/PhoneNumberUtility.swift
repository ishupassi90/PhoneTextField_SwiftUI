...
public final class PhoneNumberUtility {
    ...
    public class func defaultRegionCode() -> String {
        ...
        if #available(iOS 12.0, macOS 10.13, macCatalyst 13.1, watchOS 4.0, *) {
            ...
            if regex.firstMatch(in: countryCode, range: .init(location: 0, length: countryCode.count)) != nil {
                return countryCode
            }
        }
        #endif
        ...
        if #available(iOS 17.0, tvOS 17.0, macOS 14.0, macCatalyst 17.0, watchOS 10.0, *),
           ...
           regex.firstMatch(in: regionCode, range: .init(location: 0, length: regionCode.count)) != nil {
            return regionCode.uppercased()
        }
        #endif

        if let countryCode = (locale as NSLocale).object(forKey: .countryCode) as? String,
           regex.firstMatch(in: countryCode, range: .init(location: 0, length: countryCode.count)) != nil {
    ...
    public static func defaultMetadataCallback() throws -> Data? {
        guard
            let jsonPath = Bundle.main.path(forResource: "CountryCodeMetaData", ofType: "json"),
